...
### THOUGHT PROCESS
```
With a quick Google research a prime number is
Definition: A whole number greater than 1 that is only divisible by 1 and itself.
Divisors: Exactly two divisors (1 and the number itself).
The Number 1: 1 is not a prime number because it only has one factor.
Even Prime: 2 is the only even prime number. All other even numbers are divisible by 2, making them composite.
Infinitude: There are an infinite number of prime numbers.

Could I use the remainder for this? Let's try it out.
```

### SOLUTION
```
Well my thought was not entirely wrong, I just needed to have a few more conditions to check for other use cases. The solution below is provided by Gemini.
```
...
### THOUGHT PROCESS
Maybe we can try to use set for this, and then just compare it with the length of itself.
...
### THOUGHT PROCESS
```
Can I use the same method as implemented in Question 2 with a slight modification to the code? Let's try it out. Based on the readme on Day 2, I could check the type of each item in the list. Also in Day 4 there is a string method call isalnum() which check the alphaneumeric character, I could also try this as well.
```

### SOLUTION
```
Well I was wrong to check using isalnum(), in a real world use case I cannot directly compare to if the data is an alpahnuemeric character, it could be either one, instead what Gemini suggest is to get the type of the first data.
Then compare it with the remaining data in the list.
```
...
### THOUGHT PROCESS
```
This is a tricky one and hard for me to even imagine where to start, could this be using a library? Does this mean I need to compare if the variable is not a python variable such as comparing string and str? I have to be honest I'm thinking if it can check if the parameter provided can be a valid variable name but to check if it's a valid python variable, I will proceed with Gemini.
```

```
Even asking Gemini the same question, it is suggesting to check if the parameter is a valid variable name. By following the course structure and not using any libraries, Gemini even provded reserved keywords that is used in python to not allow any of these keywords to be used as a variable name.
```
...
### THOUGHT PROCESS
```
This is a bit tricky, so I created a new file called countries_data.py in the Day 11 folder. Since the data exist in a different file, this is the only time I will have to import a custom library.
```
